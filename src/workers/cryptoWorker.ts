...
self.onmessage = async (e: MessageEvent<WorkerInput>) => {
  const { mode, chunks, password, encryptionMode, filename, isTextMode } =
    e.data
    ...
    if (mode === 'encrypt') {
      ...
      if (isTextMode) {
        ...
        const resultArray = new Uint8Array(
          marker.length + salt.length + iv.length + encryptedData.length,
        )
        ...
        const marker = new Uint8Array([0x02])
        let totalLength =
          1 + 1 + nameLength + 1 + extensionLength + salt.length + iv.length
        ...
        for (const chunk of encryptedChunks) {
          ...
          resultArray.set(
            new Uint8Array(new Uint32Array([chunkLength]).buffer),
            offset,
          )
          offset += 4
        ...
        self.postMessage({ progress: 100, stage: 'Complete!' })
        self.postMessage({
          data: { data: resultArray.buffer, filename: filename + '.enc' },
        })
      }
      ...
      if (combinedData[0] !== 0x02)
        throw new Error('Invalid file format: Not a password-encrypted file')

      if (isTextMode) {
        self.postMessage({
          progress: 30,
          stage: 'Extracting encryption parameters...',
        })
        ...
        self.postMessage({
          progress: 40,
          stage: 'Deriving key from password...',
        })
        const key = argon2id(password, salt, argonOpts)
        const aes = gcm(key, iv)
        ...
        try {
          ...
          self.postMessage({
            progress: 90,
            stage: 'Preparing decrypted text...',
          })
        } catch (err) {
        ...
        if (combinedData.length < offset + 1)
          throw new Error('Invalid file format: File too small')
        ...
        if (offset + 1 > combinedData.length)
          throw new Error('Invalid file format: Missing extension length')
        ...
        originalExtension = new TextDecoder().decode(
          combinedData.slice(offset, offset + extensionLength),
        )
        offset += extensionLength
        ...
        self.postMessage({
          progress: 50,
          stage: 'Extracting encrypted data...',
        })
        ...
        while (offset < combinedData.length) {
          ...
          const chunkLength = new Uint32Array(
            combinedData.slice(offset, offset + 4).buffer,
          )[0]
          offset += 4
        ...
        for (let i = 0; i < encryptedChunks.length; i++) {
          try {
            ...
            console.error('Decryption error:', err)
            throw new Error(
              'Decryption failed: Invalid password or corrupted chunk',
            )
          }
        }
      ...
      self.postMessage({ progress: 100, stage: 'Complete!' })
      self.postMessage({
        data: { data: resultArray.buffer, filename, originalExtension },
      })
    } else {
